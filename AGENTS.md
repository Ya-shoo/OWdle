...
## Conventions
- **Nothing may LOOK translucent. Design rule, not an opacity rule.** Banned regardless of implementation: alpha fills (`bg-accent/10`, `bg-correct/10`, `bg-inset/40`, `border-accent/40`, `backdrop-blur` frosting) AND faint solid tints (a low-% `color-mix` body still reads as a wash and was rejected the same way). The approved language: bodies are plain `bg-card` / `bg-muted` panels with `border-line`; state is carried by a full-strength colored border (`border-correct`, `border-edge`), a solid saturated chip (`bg-accent` / `bg-info` circle with `text-on-*` glyph), or colored text, never by tinting the body. Primary buttons are solid saturated fills (`bg-accent text-on-accent`, `bg-info text-on-info`); secondary buttons are solid `bg-muted`/`bg-card` + `border-line`. On the near-black canvas `bg-inset`/`bg-surface` read as ghost boxes: reserve them for wells nested INSIDE a card and for full-width bands (footer), and use `bg-muted` or `bg-card` for anything free-standing. Alpha is fine ONLY for shadows/glows and gradient scrims. `text-wrong` is a tile FILL color (dark navy), not a text color; labels on dark surfaces use `text-on-wrong`. **The ONE sanctioned body-color exception:** the end-of-round WIN / LOSS result cards carry the outcome in the body itself via the solid `bg-win` (deep green) / `bg-loss` (soft red) tokens, paired with `border-correct` / `border-loss-edge`. These are full-strength OPAQUE fills, never tints, so the no-translucency rule still holds. Result-card bodies ONLY (`LossReveal`, each `*Game` win card, `DailyCompleteResultCard`, Melee/Archive result cards) — plus the one further case of the Melee/Ability avatar guess cards, which are borderless `rounded-(--radius-card)` cards that likewise carry state in the body: a solid `bg-loss` red for a wrong hero, navy `bg-card` for the correct guess, with Melee's role chip carrying the role match on top.
