...
// otherwise null. Never throws — a missing secret puts the server into
// a degraded "viewer offline" mode rather than crashing it, so the
// unified `npm run dev` doesn't die when ADMIN_SECRET isn't provisioned
// on this machine. Configure later by creating .env.secrets and
// restarting.
function readSecret() {
...
<head>
<meta charset="utf-8" />
<title>Votes admin · ADMIN_SECRET needed</title>
...
<main>
  ...
  <h1>Set <code>ADMIN_SECRET</code> to view votes</h1>
  <p>This dashboard reads from <code>${ORIGIN}/api/votes</code>, which is
  ...
  <p class="muted">The rest of the dev hub works without this — it only matters when you want to read
     prod vote data. Local-only file; never committed (gitignored via <code>.env*</code>).</p>
</main>
</body>
</html>`;
