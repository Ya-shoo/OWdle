...
// to the home page after the meta-refresh interval. That's a couple
// hundred ms longer than a 30x redirect, but a meta refresh keeps the
// OG meta tags in the same response so the unfurler doesn't follow
// the redirect to scrape OG from /. (Some unfurlers do follow 30x,
// some don't; meta refresh sidesteps the variance entirely.)
//
...
export const onRequestGet: Handler = async ({ params, request }) => {
  ...
  const total = decoded.results.length;
...
<body>
<a href="${escapeHtml(homeUrl)}">Continue to OWdle →</a>
</body>
</html>`;
